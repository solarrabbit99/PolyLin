...
do
  ...
  numOp=$(( 10000 * $i ))
	for j in {1..3};
	do
		./prodcon-bs-kfifo -producers=2 -consumers=2 -operations=$numOp -log_operations=1 -print_summary=0 -shuffle_threads=1 > temp.log
		./format_history.py temp.log bs-kfifo.$i$j.log
		rm -f temp.log
	done
done
