...
// ############################################################################################################
enum class rb_color { fail, red, black, double_black };
// ############################################################################################################
...
constexpr
  ...
  return interval<numerical_type, interval_kind_>{std::min(lhs, rhs),
                                                  std::max(lhs, rhs)};
}
// ############################################################################################################
template <typename numerical_type = default_interval_value_type,
...
class node {
 ...
 public:
  ...
  friend lib_interval_tree::const_interval_tree_iterator<
      node<numerical_type, interval_type>>;
  friend lib_interval_tree::interval_tree_iterator<
      node<numerical_type, interval_type>>;
 ...
 public:
  ...
  bool is_left() const noexcept { return this == parent_->left_; }

  bool is_right() const noexcept { return this == parent_->right_; }

  bool is_root() const noexcept { return !parent_; }

  /**
   *  Returns the color of the node.
   */
  rb_color color() const { return color_; }
  ...
  /**
   *  Returns the height of the node in the tree. Where height = how many
   * parents does it have. The root has no parents and is therefor has height 0.
   */
  int height() const {
    int counter{0};
    for (auto* p = parent_; p != nullptr; p = p->parent_) ++counter;
    return counter;
  }
   ...
   */
  value_type low() const { return interval_.low(); }

  /**
   *  Returns the upper bound of the interval of this node
   */
  value_type high() const { return interval_.high(); }

 private:
  void set_interval(interval_type const& ival) { interval_ = ival; }
...
class basic_interval_tree_iterator : public std::forward_iterator_tag {
 public:
  ...
  using node_ptr_t = typename std::conditional<
      ...
      node_type const*, node_type*>::type;

 public:
  constexpr basic_interval_tree_iterator(basic_interval_tree_iterator const&) =
      default;
  constexpr basic_interval_tree_iterator(
      basic_interval_tree_iterator&& other) noexcept = default;
  basic_interval_tree_iterator& operator=(basic_interval_tree_iterator const&) =
      default;
   ...
   *  Returns the color of the node.
   */
  rb_color color() const { return node_->color(); }
...
class const_interval_tree_iterator
    : public basic_interval_tree_iterator<
          node_type, interval_tree<typename node_type::interval_type> const*> {
 public:
  using tree_type = interval_tree<typename node_type::interval_type>;
  using iterator_base =
      basic_interval_tree_iterator<node_type, tree_type const*>;
 ...
 public:
  ...
  constexpr const_interval_tree_iterator(const_interval_tree_iterator const&) =
      default;
  constexpr const_interval_tree_iterator(
      const_interval_tree_iterator&&) noexcept = default;
  const_interval_tree_iterator& operator=(const_interval_tree_iterator const&) =
      default;
  ...
  typename value_type::interval_type operator*() const {
    if (node_)
      ...
      throw std::out_of_range(
          "dereferencing interval_tree_iterator out of bounds");
  }
  ...
  value_type const* operator->() const { return node_; }
...
class interval_tree_iterator
    : public basic_interval_tree_iterator<
          node_type, interval_tree<typename node_type::interval_type>*> {
 public:
  using tree_type = interval_tree<typename node_type::interval_type>;
 ...
 public:
  ...
  interval_tree_iterator& operator=(interval_tree_iterator&&) noexcept =
      default;
  ...
  value_type* operator->() { return node_; }
...
class interval_tree {
 ...
 public:
  interval_tree() : root_{nullptr}, size_{0} {}

  ~interval_tree() { clear(); }

  interval_tree(interval_tree const& other) : root_{nullptr}, size_{0} {
    operator=(other);
  }
  ...
  interval_tree& operator=(interval_tree const& other) {
    ...
    if (other.root_ != nullptr) root_ = copy_tree_impl(other.root_, nullptr);
   ...
   */
  iterator root() { return {root_, this}; }

  /**
   *  Returns the root node from this tree.
   */
  const_iterator root() const { return {root_, this}; }
   ...
   *  @param mergeSetOverlapping If the result of interval::join is a collection
   * of intervals, shall each be inserted with more overlap searches?
   */
  iterator insert_overlap(interval_type const& ival, bool exclusive = false,
                          bool mergeSetOverlapping = false) {
  ...
  iterator erase(iterator iter) {
    if (!iter.node_) throw std::out_of_range("cannot erase end iterator");
  ...
  const_iterator find(interval_type const& ival,
                      CompareFunctionT const& compare) const {
    if (root_ == nullptr) return end();
  ...
  template <typename CompareFunctionT>
  const_iterator find_next_in_subtree(iterator from, interval_type const& ival,
                                      CompareFunctionT const& compare) const {
    if (root_ == nullptr) return end();
    return iterator{find_i_ex(from.node_, ival, compare), this};
  }
  ...
  const_iterator find_next_in_subtree(iterator from,
                                      interval_type const& ival) const {
    return find_next_in_subtree(
        from, ival,
        [](auto const& lhs, auto const& rhs) { return lhs == rhs; });
  }
   ...
   */
  template <typename FunctionT>
  void overlap_find_all(interval_type const& ival, FunctionT const& on_find,
                        bool exclusive = false) {
    if (root_ == nullptr) return;
    if (exclusive)
      overlap_find_all_i<this_type, true, iterator>(this, root_, ival, on_find);
    else
      overlap_find_all_i<this_type, false, iterator>(this, root_, ival,
                                                     on_find);
  }
  template <typename FunctionT>
  void overlap_find_all(interval_type const& ival, FunctionT const& on_find,
    ...
    if (root_ == nullptr) return;
    if (exclusive)
      overlap_find_all_i<this_type, true, const_iterator>(this, root_, ival,
                                                          on_find);
    else
      overlap_find_all_i<this_type, false, const_iterator>(this, root_, ival,
                                                           on_find);
  }
  ...
  interval_tree deoverlap_copy() {
    interval_tree fresh;
    for (auto i = begin(), e = end(); i != e; ++i) fresh.insert_overlap(*i);

    return fresh;
  }

  /**
   *  Merges all overlapping intervals by erasing overlapping intervals and
   * reinserting the merged interval.
   */
   ...
   *  Creates an interval tree that contains all gaps between the intervals as
   * intervals.
   */
  ...
  /**
   *  Only works with deoverlapped trees.
   *  Removes all intervals from the given interval and produces a tree that
   * contains the remaining intervals. This is basically the other punch
   * overload with ival = [tree_lowest, tree_highest]
   */
  ...
  iterator begin() {
    if (!root_) return {nullptr, this};

    auto* iter = root_;

    while (iter->left_) iter = iter->left_;
 ...
 private:
  ...
  iterator insert_merge_set(MergeSet const& merge_set,
                            bool mergeSetOverlapping) {
  ...
  template <typename ThisType, typename IteratorT, typename FunctionT,
            typename ComparatorFunctionT>
  static bool find_all_i(
      typename std::conditional<std::is_same<IteratorT, iterator>::value,
                                ThisType, ThisType const>::type* self,
      node_type* ptr, interval_type const& ival, FunctionT const& on_find,
      ComparatorFunctionT const& compare) {
    if (compare(ptr->interval(), ival)) {
      if (!on_find(IteratorT{ptr, self})) return false;
    }
    if (ptr->left_ && ival.high() <= ptr->left_->max()) {
      // no right? can only continue left
      if (!ptr->right_ || ival.low() > ptr->right_->max())
        return find_all_i<ThisType, IteratorT>(self, ptr->left_, ival, on_find,
                                               compare);

      if (!find_all_i<ThisType, IteratorT>(self, ptr->left_, ival, on_find,
                                           compare))
        return false;
    }
    if (ptr->right_ && ival.high() <= ptr->right_->max()) {
      if (!ptr->left_ || ival.low() > ptr->left_->max())
        return find_all_i<ThisType, IteratorT>(self, ptr->right_, ival, on_find,
                                               compare);

      if (!find_all_i<ThisType, IteratorT>(self, ptr->right_, ival, on_find,
                                           compare))
        return false;
    }
    return true;
  ...
  node_type* find_i(node_type* ptr, interval_type const& ival,
                    ComparatorFunctionT const& compare) const {
    if (compare(ptr->interval(), ival))
      return ptr;
    else
  ...
  // excludes ptr
  template <typename ComparatorFunctionT>
  node_type* find_i_ex(node_type* ptr, interval_type const& ival,
                       ComparatorFunctionT const& compare) const {
    if (ptr->left_ && ival.high() <= ptr->left_->max()) {
      // no right? can only continue left
      if (!ptr->right_ || ival.low() > ptr->right_->max())
...
#endif
    {
      if (ptr->interval().overlaps_exclusive(ival)) return ptr;
    } else {
      if (ptr->interval().overlaps(ival)) return ptr;
    }

    return overlap_find_i_ex<Exclusive>(ptr, ival);
  }

  template <typename ThisType, bool Exclusive, typename IteratorT,
            typename FunctionT>
  static bool overlap_find_all_i(
    ...
    if (ptr->left_ && ptr->left_->max() >= ival.low()) {
      ...
      if (!ptr->right_ || ival.low() > ptr->right_->max())
        return overlap_find_all_i<ThisType, Exclusive, IteratorT>(
            self, ptr->left_, ival, on_find);

      if (!overlap_find_all_i<ThisType, Exclusive, IteratorT>(self, ptr->left_,
                                                              ival, on_find))
        return false;
    }
    if (ptr->right_ && ptr->right_->max() >= ival.low()) {
      ...
      if (!overlap_find_all_i<ThisType, Exclusive, IteratorT>(self, ptr->right_,
                                                              ival, on_find))
        return false;
    }
    return true;
  ...
  // excludes ptr
  template <bool Exclusive>
  node_type* overlap_find_i_ex(node_type* ptr,
                               interval_type const& ival) const {
    if (ptr->left_ && ptr->left_->max() >= ival.low()) {
      // no right? can only continue left
  ...
  node_type* successor(node_type* node) {
    if (node->right_) return minimum(node->right_);
    auto* y = node->parent_;
   ...
   *  Creates orphaned nodes. A transplant call must be succeeded by delete
   * calls.
   */
  ...
  void left_rotate(node_type* x) {
    ...
    // max fixup
    if (x->left_ && x->right_)
      x->max_ = std::max(x->interval_.high(),
                         std::max(x->left_->max_, x->right_->max_));
    else if (x->left_)
    ...
    if (y->right_)
      y->max_ =
          std::max(y->interval_.high(), std::max(y->right_->max_, x->max_));
    else
  ...
  void right_rotate(node_type* y) {
    ...
    // max fixup
    if (y->left_ && y->right_)
      y->max_ = std::max(y->interval_.high(),
                         std::max(y->left_->max_, y->right_->max_));
    else if (y->left_)
    ...
    if (x->left_)
      x->max_ =
          std::max(x->interval_.high(), std::max(x->left_->max_, y->max_));
    else
  ...
  void recalculate_max(node_type* reacalculation_root) {
    ...
    while (p && p->max_ <= reacalculation_root->max_) {
      if (p->left_ && p->left_->max_ > p->max_) p->max_ = p->left_->max_;
      if (p->right_ && p->right_->max_ > p->max_) p->max_ = p->right_->max_;
      p = p->parent_;
    }
  }
